...
public class ImageTools {
	...
	public static BufferedImage applyEnhancedColors(BufferedImage b,
			final int dark, final int bright) {
		IModify ef = new IModify() {
			public int modify(int red, int green, int blue, int alpha,
					int layerRed, int layerGreen, int layerBlue, int layerAlpha) {
	...
	public static BufferedImage applySepia(BufferedImage b) {
		IModify ef = new IModify() {
			public int modify(int red, int green, int blue, int alpha,
					int layerRed, int layerGreen, int layerBlue, int layerAlpha) {
				int newR = Math
						.round((float) ((red * 0.393) + (green * 0.769) + (blue * 0.189)));
				int newG = Math
						.round((float) ((red * 0.349) + (green * 0.686) + (blue * 0.168)));
				int newB = Math
						.round((float) ((red * 0.272) + (green * 0.534) + (blue * 0.131)));
	...
	public static BufferedImage applyLayer(BufferedImage b, BufferedImage layer) {
		IModify ef = new IModify() {
			public int modify(int red, int green, int blue, int alpha,
				...
				// Calculate distance from original color value to layer color
				// value
				int dr = Math.abs(red - layerRed);
				int dg = Math.abs(green - layerGreen);
				int db = Math.abs(blue - layerBlue);

				// Calculate distance to move based on layer transparency
				// percent
