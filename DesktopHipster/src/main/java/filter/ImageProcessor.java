...
public class ImageProcessor {
	...
	public static BufferedImage modifyPixels(BufferedImage b, IModify m, BufferedImage layer) {
		return applyPixelChanges(b,m,layer);
	}
	
	private static BufferedImage applyPixelChanges(BufferedImage b, IModify m, BufferedImage layer) {
		...
		for(int w = 0; w < width; w++) {
			for(int h = 0; h < height; h++) {
				...
				int newColor = m.modify(red, green, blue, alpha, eR, eG, eB, eA);
