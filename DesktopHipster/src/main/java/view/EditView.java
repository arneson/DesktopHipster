 ...
 * stage where the user edits his/hers image.
 * 
...
public class EditView extends Card implements PropertyChangeListener {
	...
	private ActionListener filterButtonClick = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			pcs.firePropertyChange(PropertyNames.VIEW_ACTIVE_FILTER_CHANGE,
					null, ((FilterButton) e.getSource()).getFilter());
			/*
			 * pcs.firePropertyChange(PropertyNames.REQUEST_CHANGE_CARD_VIEW,
			 * null, ((FilterButton)e.getSource()).getFilter());
			 */
		}
	};
	...
	public void initialize() {
		...
		addWest(new JPanel(new BorderLayout()) {
			{
				add(backButton, BorderLayout.CENTER);
			}
		});
		addCenter(new JPanel() {
			{
				add(canvas);
			}
		});
		...
		case PropertyNames.MODEL_ACTIVE_IMAGE_CHANGE:
			if (evt.getNewValue() != null) {
				canvas.setIcon(new ImageIcon(
						((ExtendedImage) evt.getNewValue()).getPreview()));
				revalidate();
				repaint();
			}
			break;
